...
- **Deploys are automatic on push, not manual.** Both Vercel and Railway are connected via native GitHub integration (`docs/logbook.md` #17-#19) — `git push origin main` deploys both platforms within seconds. `railway up` / `vercel --prod` still work as manual overrides but are no longer the normal workflow; don't reach for them by default. Railway also has `NIXPACKS_UV_VERSION` pinned as a service variable (currently `0.4.30`) — without it, Nixpacks resolves `uv`'s version via a live external lookup that can transiently fail and break the build; bump this variable manually if `uv` ever needs upgrading, it won't update itself. See `DEPLOY.md` §0 for the full one-time setup (Root Directory settings on both platforms matter — this is a monorepo).
