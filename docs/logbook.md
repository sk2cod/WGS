...
**Not a blueprint deviation.** The export-confirmation event, real content persistence, and voice-compounding cap are new functionality implementing what blueprint.md Section 4 and Section 11 already described but was never wired up (#31's own framing) — not a departure from anything locked. The `upsert_brand_kit()` fix, `brand_kit_singleton_idx` safeguard, and the training-retry decoupling are bug fixes and correctness hardening, the same classification as #34's RLS work, not a design decision.
