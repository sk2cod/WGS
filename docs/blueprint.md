...
**Note:** as of logbook #39, carousel is at 6 slides for v1 (cover, 3 body, closing, conversation), not 3–4 — round 7 added `carousel_conversation` after the closing (the first structural, not prompt-only, change in the v1 content-voice experiment), then round 8 raised body slides 1–2 → 3, fixed regardless of approach, with new anti-padding/split guidance added specifically to keep the extra room from being padded. See the logbook for why and current status.
...
**Further note:** as of logbook #39 round 8, `carousel_conversation` also carries the `cta`/`handle` footnote (the "Follow us..." sentence and the full `@handle`) — moved here from C3's closing slide, where they'd been landing as a leftover from before this template existed. C3 now renders only its `takeaway`; the `signature` ("with you,") value still computes on the backend but is no longer rendered anywhere, a display-only removal (same pattern as #32's masthead simplification).
